...
from test_framework.test_framework import HempyCoinTestFramework
...
class ReorgStakeTest(HempyCoinTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_zhyc):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected HYC and zHYC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zHYCsupply']:
                assert_equal(node['zHYCsupply'][denom], DecimalAmt(expected_zhyc[denom]))
    ...
    def run_test(self):
        ...
        # Check HYC and zHYC supply at the beginning
        # ------------------------------------------
        # zHYC supply: 2 coins for each denomination
        expected_zhyc_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # HYC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zhyc_supply)
        ...
        # Verify that HYC and zHYC supplies were properly updated after the spends and reorgs
        self.log.info("Check HYC and zHYC supply...")
        ...
        expected_zhyc_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zhyc_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zhyc_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zhyc_supply)
