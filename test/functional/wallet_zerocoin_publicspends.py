...
class ZerocoinSpendTest(HempyCoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zhyc_bal, bc_bal):
            zhyc_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zhyc_bal)
            bc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], bc_bal)
            return zhyc_bal, bc_bal
        ...
        zhyc_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zhyc_balance, 6666)
        ...
        zhyc_balance, balance = check_balances(denom_2, zhyc_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zhyc_balance, balance = check_balances(denom_3, zhyc_balance, balance)
        ...
        _, _ = check_balances(denom_1, zhyc_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
