UNIX BUILD NOTES
====================
Some notes on how to build HempyCoin in Unix.

Note
---------------------
Always use absolute paths to configure and compile HempyCoin and the dependencies,
...
This will build hempycoin-qt as well, if the dependencies are met.
...
memory available when compiling HempyCoin. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~hempycoin/+archive/hempycoin).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:hempycoin/hempycoin
    sudo apt-get update
...
To build HempyCoin without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build hempycoin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a hempycoin-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip hempycoind" to strip the debug
...
Security
--------
To help make your HempyCoin installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, HempyCoin should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, HempyCoin may be compiled in
