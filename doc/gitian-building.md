...
*Setup instructions for a gitian build of HempyCoin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the HempyCoin
...
to the HempyCoin GitHub release page.
...
- [Building HempyCoin](#building-hempycoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/hempycoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/hempycoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/hempycoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/hempycoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/hempycoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/hempycoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/hempycoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@hempycoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/hempycoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/hempycoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/hempycoin-osx-*-build.assert
```
