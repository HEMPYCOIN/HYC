...
This document provides guidelines for internationalization of the HempyCoin software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to HempyCoin network</source>
        <translation>
            <numerusform>%n active connection to HempyCoin network</numerusform>
            <numerusform>%n active connections to HempyCoin network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `hempycoin_en.ts` remains unchanged.
