...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/HempyCoinParams"
    else
        PARAMS_DIR="$HOME/.hempycoin-params"
    fi
fi
...
Failed to fetch the HempyCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
HempyCoin - fetch-params.sh

This script will fetch the HempyCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common HempyCoin zkSNARK parameters. Note that it is
