...
#else
    const char* pszModule = "hempycoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\HempyCoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\HempyCoin
// Mac: ~/Library/Application Support/hempycoin
// Unix: ~/.hempycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HempyCoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HempyCoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HempyCoinParams
    // Mac: ~/Library/Application Support/HempyCoinParams
    // Unix: ~/.hempycoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HempyCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "HempyCoinParams";
#else
    // Unix
    return pathRet / ".hempycoin-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty hempycoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override hempycoin.conf
