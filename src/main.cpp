...
#if defined(NDEBUG)
#error "HempyCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uhyc) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("hempycoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
		...
		//if (fEnforceBIP30) {
		//	const Coin* coins = view.AccessCoins(tx.GetHash());
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> hycInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                hycInputs.push_back(stakeIn);
        }
        const bool hasHYCInputs = !hycInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasHYCInputs) {
                    ...
                    for (const CTxIn& hycIn : hycInputs)
                        if(hycIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasHYCInputs) {
                            for (const CTxIn& stakeIn : hycInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // HempyCoin: We use certain sporks during IBD, so check to see if they are
