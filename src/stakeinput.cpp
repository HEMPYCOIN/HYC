...
bool CHycStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CHycStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CHycStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CHycStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CHycStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CHycStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CHycStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CHycStake::GetUniqueness() const
{
    //The unique identifier for a HYC stake is the outpoint
...
CBlockIndex* CHycStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CHycStake::ContextCheck(int nHeight, uint32_t nTime)
{
