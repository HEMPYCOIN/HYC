...
#include "qt/hempycoin/forms/ui_snackbar.h"
#include "qt/hempycoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(HempyCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &HempyCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
