...
#include "qt/hempycoin/settings/forms/ui_settingsinformationwidget.h"
...
SettingsInformationWidget::SettingsInformationWidget(HempyCoinGUI* _window,QWidget *parent) :
    ...
    ui(new Ui::SettingsInformationWidget)
{
	...
	setCssBtnSecondary(ui->pushButtonMNFile);
    ...
    connect(ui->pushButtonFile, &QPushButton::clicked, [this](){
        if (!GUIUtil::openConfigfile())
            inform(tr("Unable to open hempycoin.conf with default application"));
    });
	connect(ui->pushButtonMNFile, &QPushButton::clicked, [this]() {
