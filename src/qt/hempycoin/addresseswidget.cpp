...
#include "qt/hempycoin/forms/ui_addresseswidget.h"
#include "qt/hempycoin/addresslabelrow.h"
#include "qt/hempycoin/addnewaddressdialog.h"
#include "qt/hempycoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(HempyCoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination hycAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(hycAdd, label.toUtf8().constData(),
