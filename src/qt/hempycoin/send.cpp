...
#include "qt/hempycoin/forms/ui_send.h"
#include "qt/hempycoin/addnewcontactdialog.h"
#include "qt/hempycoin/qtutils.h"
#include "qt/hempycoin/sendchangeaddressdialog.h"
#include "qt/hempycoin/optionbutton.h"
#include "qt/hempycoin/sendconfirmdialog.h"
#include "qt/hempycoin/myaddressrow.h"
#include "qt/hempycoin/guitransactionsutils.h"
...
SendWidget::SendWidget(HempyCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-hyc");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination hycAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(hycAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(hycAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
