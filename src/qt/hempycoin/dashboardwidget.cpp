...
#include "qt/hempycoin/dashboardwidget.h"
#include "qt/hempycoin/forms/ui_dashboardwidget.h"
#include "qt/hempycoin/sendconfirmdialog.h"
#include "qt/hempycoin/txrow.h"
...
DashboardWidget::DashboardWidget(HempyCoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareHyc, "square-chart-hyc");
	setCssProperty(ui->labelHyc, "text-chart-hyc");
	...
	setCssProperty(ui->labelAmountHyc, "text-stake-hyc-disable");
	...
	if (window)
		connect(window, &HempyCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, &HempyCoinGUI::openFAQ);
		connect(ui->btnHowTo, &QPushButton::clicked, window, &HempyCoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair HYC
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			hyc = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalHyc += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesHyc.append(hyc);

		int max = std::max(hyc, hyc);
...
void DashboardWidget::onChartRefreshed()
{
	...
	set0->setColor(QColor(163, 132, 83));
	...
	set0->append(chartData->valuesHyc);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalHyc > 0) {
		setCssProperty(ui->labelAmountHyc, "text-stake-hyc");
	}
	else {
		setCssProperty(ui->labelAmountHyc, "text-stake-hyc-disable");
	}
	forceUpdateStyle({ ui->labelAmountHyc });
	ui->labelAmountHyc->setText(GUIUtil::formatBalance(chartData->totalHyc, nDisplayUnit));
